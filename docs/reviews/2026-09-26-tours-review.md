...
## Method & limits
- **The probe.** A throwaway SwiftPM executable at `/private/tmp/claude-501/…/scratchpad/agents/tours-review/probe/`. It is a temporary folder and may be gone. linked to the repo's six packages by path. `sync.sh` copies all of `App/` except `Main.swift`, with probe-only patches: windows parked at desktop level +2 and never ordered front or activated, the mic permission status faked, settings in a probe suite, History in scratch.
